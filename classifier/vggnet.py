...
class VggNet(object):
    ...
    def build_16(path_npy, batch_tensors, end_layer='@_@'):
        ...
        """
        return VggNet('vgg_16', path_npy, batch_tensors, end_layer)

    @staticmethod
    def build_19(path_npy, batch_tensors, end_layer='@_@'):
        """
        Virtual constructor to construct a VGG 19 (E) net.
        """
        return VggNet('vgg_19', path_npy, batch_tensors, end_layer)
    ...
    def fc(weights, tensors, name):
        ...
        with tf.variable_scope(name):
            ...
            tensors = tf.contrib.layers.flatten(tensors)
            tensors = tf.matmul(tensors, const_weights)
            tensors = tf.nn.bias_add(tensors, const_biases)

            return tensors

    def __init__(self, model, path_npy, batch_tensors, end_layer):
        """
        ...
        for layer_name in layer_names:
            if layer_name == end_layer:
                ...
                batch_tensors = VggNet.conv(weights, batch_tensors, layer_name)
            elif layer_name.startswith('pool'):
                batch_tensors = VggNet.pool(batch_tensors, layer_name)
            elif layer_name.startswith('fc'):
                batch_tensors = VggNet.fc(weights, batch_tensors, layer_name)
